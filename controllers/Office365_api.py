...
class SharePoint:
    def __init__(self, sp_site, uname, passw, sp_primary_directory):

        self.SHAREPOINT_DOC = sp_primary_directory
        self.SHAREPOINT_SITE = sp_site
        self.USERNAME = uname
        self.PASSWORD = passw
    ...
    #     conn = self._auth()
    #     file_url = f'/sites/{self.SHAREPOINT_SITE_NAME}/{SHAREPOINT_DOC}/{folder_name}/{file_name}'
    ...
    def upload_file(self, file_name, folder_name, content):
        conn = self._auth()
        target_folder_url = f'/sites/{self.SHAREPOINT_SITE_NAME}/{SHAREPOINT_DOC}/{folder_name}'
